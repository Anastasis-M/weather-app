...
<div class="relative w-full">
  <div class="flex items-center gap-2 rounded-2xl bg-panel hairline px-3 h-11">
    ...
    <input
      ...
      class="flex-1 bg-transparent outline-none text-[15px] placeholder:text-mute"
      autocomplete="off"
      enterkeyhint="search"
    />
    <button
      type="button"
      onclick={() => onLocate?.()}
      class="p-1.5 rounded-lg text-sub hover:text-ink hover:bg-panel2 transition"
    ...
    >
      ...
      {#each results as r, i}
        <li>
          <button
            type="button"
            class="w-full text-left px-3 py-2.5 flex items-baseline gap-2 hover:bg-panel2 transition {i === active ? 'bg-panel2' : ''}"
            onclick={() => pick(r)}
